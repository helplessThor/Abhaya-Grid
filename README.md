<![CDATA[# 🛡️ Abhaya Grid — National Women's Safety Matrix
...
- All 35 Indian States & Union Territories rendered as interactive polygons
...
- Powered by OpenStreetMap's **Nominatim** geocoding API
- Cinematic `flyTo` animation transitions the camera to the target area

### 📊 Crime Data Visualization
- **~78,000+ geospatial incident records** scattered across India
- Heatmap layer for density visualization
- Individual clickable crime points with detailed **Incident Report** popups showing:
  - Crime type (Rape, Kidnapping, Dowry Death, etc.)
...
- Dynamically updates as the user pans/zooms the map
...
├── data_ingestion/
│   └── fetch_crime_data.py        # NCRB data fetcher & GeoJSON generator
├── public/
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/YOUR_USERNAME/abhaya-grid.git
   cd abhaya-grid
   ```
...
3. **Run the data ingestion pipeline**
   ...
   ```
   This will:
   - Fetch the official NCRB dataset from a verified public source
   - Download Indian state boundary GeoJSON
   - Generate ~78,000 geospatially scattered crime records
   - Save `abhaya.db` (SQLite) and `public/india_states_crimes.geojson`
...
| `timeWindow` | `string` | `1w`, `1m`, `6m`, `1y`, `2y`, `3y` |
...
|--------|-------------|-----|
| **NCRB Dataset** | Crimes Against Women in India (2001-2012) | [GitHub - tvganesh/crime-against-women](https://github.com/tvganesh/crime-against-women) |
| **State Boundaries** | GeoJSON polygons of Indian States/UTs | [GitHub - Subhash9325/GeoJson-Data-of-Indian-States](https://github.com/Subhash9325/GeoJson-Data-of-Indian-States) |
| **Map Tiles** | Dark-mode basemap | [CARTO Dark Matter (No Labels)](https://carto.com/basemaps) |
| **Geocoding** | Location search | [Nominatim (OpenStreetMap)](https://nominatim.openstreetmap.org) |
| **Safe Zones** | Police & Pharmacy POIs | [Overpass API (OpenStreetMap)](https://overpass-api.de) |
...
When the dashboard loads, you see the entire nation colored by crime intensity. Darker states = higher crime volumes. This uses a `fill` layer on the GeoJSON polygons with an interpolated color scale.

### Micro View (City/Neighborhood Level)
When you search for a location or click a state, the map smoothly zooms in. The state polygons fade out, and the individual crime data points fade in as a heatmap + clickable circles.
...
<p align="center">
  <strong>Built with purpose. Designed for safety.</strong>
  <br/>
  <em>Abhaya (अभया) — Sanskrit for "Fearless"</em>
</p>
]]>
